...
const create = function () {
    ...
    for (let i = 1; i <= 100; i++) {
        ...
        main.appendChild(newDiv).setAttribute("myid" + i, "div_" + i);
...
inputMin.onkeyup = function (k) {
    ...
    if (!((k.keyCode > 95 && k.keyCode < 106)
        || (k.keyCode > 47 && k.keyCode < 58)
        || [8, 13, 37, 39].indexOf(k.keyCode) >= 0
    )) {
        return false;
    }
    create();
...
$(document).ready(function(){ $("#btnSix").click(function(){ 
    $(".buzz, .fizz, .fizzbuzz").toggle();
  });
});